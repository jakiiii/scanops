...
class _BaseHistoryListView(CapabilityRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        can_view_all = data_visibility_service.user_can_view_all_data(self.request.user)
        page_title = self.page_title
        page_subtitle = self.page_subtitle
        if self.page_mode == "history" and not can_view_all:
            page_title = "My Scan History"
            page_subtitle = "Execution history owned by your account."
        ...
        context["scope_label"] = "All" if can_view_all else "My"
        context["breadcrumbs"] = [
...
class ScanHistoryView(_BaseHistoryListView):
    ...
    page_mode = "history"
    page_title = "Scan History"
    page_subtitle = "Execution history visible to your account."
...
class HistoryCompareRedirectView(CapabilityRequiredMixin, View):
    ...
    def post(self, request, pk: int) -> HttpResponse:
        queryset = data_visibility_service.get_user_visible_executions(
            request.user,
            queryset=ScanExecution.objects.select_related("result"),
        )
        execution = get_object_or_404(queryset, pk=pk)
        ...
        if result is None:
            ...
            return _redirect_back(request, reverse("scans:history"))
        previous_result = get_previous_result(result, user=request.user)
